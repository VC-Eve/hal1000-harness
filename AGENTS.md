...
- `node scripts/conditions-check.mjs` (after `npm run build`) — the verification for a slot that says *when* it is drawn: boots a throwaway HAL with a two-State World carrying an always-drawn control slot, a slot on a clause, a picture scoped to the second State and a caption with a fade, opens `/live` and `/broadcast`, flips each Parameter through the editor, and prints every overlay element's computed `display`, `visibility`, `opacity`, transition and client rect either side of every flip, plus the fade's sampled trajectory. jsdom applies no stylesheet, so a component test can assert the `hidden` attribute and nothing about what an operator sees. Needs `ffmpeg` and playwright's chromium. The measurements that matter: `display: none` on **both** surfaces with **no** `[hidden]` rule in `styles.css` — the UA rule wins unopposed because neither `.overlay-slot` nor `.overlay-image` sets `display`, and adding one would manufacture the two-cascade-positions hazard; the faded slot running 0 → 1 over its configured length while the unfaded one shows no intermediate opacity at all; the scoped picture keeping its node identity across a hide; and the control slot beneath a conditioned one moving 10px on `/live` and 52px on `/broadcast` when its neighbour appears, which is the reflow KTD8 accepts.
- `node scripts/treatment-check.mjs` (after `npm run build`) — the verification for how a slot's words are treated: boots a throwaway HAL over a flat grey clip with five captions differing only in their outline, shadow and band, opens `/live` and `/broadcast`, and prints each slot's computed `text-shadow`, stroke width and colour, `paint-order`, type size in pixels, and the stroke as a share of that type size. Needs `ffmpeg` and playwright's chromium; exits non-zero when a claim fails. The measurement that matters is the one nothing cheaper can make: a stroke changes no layout metric in any engine, so the outline claim is settled in **pixels** — the word is screenshotted on `/broadcast`, handed back to a blank page to be decoded by canvas, and its white is counted against the untreated control. Measured 2026-09-07 on chromium: 92.9% of the white kept and 5,816 black border pixels added, so `paint-order: stroke fill` paints the stroke behind the fill and the letterform survives its border. That settles the plan's one open question and the ring-of-offsets fallback is not needed. The stroke's share of the type size is 8.00 on both surfaces at 16.48px and 86.4px of type, which is the proportion claim.
...
- `shared/src/types.ts` — the single source of truth for the WS wire contract (every `ClientMessage`/`ServerMessage`) and shared data shapes. All meaningful behavior must be reachable through this protocol, never UI-only (agent-native parity rule). One socket-level capability lives here: `observe` declares that a connection only watches, and the server then refuses it the audio grant and the two clip reports. It is a message rather than a URL or a connection parameter precisely so an agent has it too. The gate is in `AudioService.attend` and `takeAuthority` — *not* in `elect`, which nothing consults before handing the grant out; see `docs/solutions/a-gate-that-checks-one-direction-is-half-a-gate.md`.
